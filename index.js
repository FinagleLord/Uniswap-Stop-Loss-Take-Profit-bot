...
// \_   _____/|__| ____ _____     ____ |  |   ____ |    |    ___________  __| _/
//  |    __)  |  |/    \\__  \   / ___\|  | _/ __ \|    |   /  _ \_  __ \/ __ | 
//  |     \   |  |   |  \/ __ \_/ /_/  >  |_\  ___/|    |__(  <_> )  | \/ /_/ | 
//  \___  /   |__|___|  (____  /\___  /|____/\___  >_______ \____/|__|  \____ | 
//      \/            \/     \//_____/           \/        \/                \/ 
...
// TRADING Amount:                  (X)  
const ETH_AMOUNT = web3.utils.toWei('0.1', 'Ether').toString();
console.log("Eth Amount", ETH_AMOUNT)

// #4:

// STOP LOSS                             (X)
const ETH_STOPLOSS_PRICE = web3.utils.toWei('382', 'Ether').toString(); // 200 Dai a.k.a. $200 USD

// #5:

// TAKE PROFIT                             (X)
const ETH_TAKEPROFIT_PRICE = web3.utils.toWei('383', 'Ether').toString(); // 1000 Dai a.k.a. $1000 USD ;)
...
// Get and set TX Gas Price (Standard by default) -------------- @Hiturunk
let gasPrice = 0;
...
function setGasPrice(response){
gasPrice = JSON.parse(response);
}
...
// GAS setting: 
// (IE SLOW, STANDARD,  FAST)                        (X)
httpGetAsync('https://www.ethgasstationapi.com/api/standard/' ,setGasPrice);
}
updatePrice();
...
async function monitorPrice() {
  ...
  try {
    ...
    if(price >= ETH_TAKEPROFIT_PRICE) {
      console.log('TAKE PROFIT MET: Selling Eth...')
      // Check balance before sale
      await checkBalances()
